...
void test1()
{
  cout<<"********* Test1 **************\n";
  {
  TPoly<float> pnull, p0(5.5f), p1(1.2f, 2.4f), p2(1.1f, -4.3f, 0.7f), p3(0,12.4f,3.7f,2.44f), p4(0,0,-5.7f,0,8.5f);
  ...
  cout<<2.0f*p1; // le compilateur essaie de transformer p1 en float et ça ne marche pas
                 // mais il ne transforme pas automatiquement 2.0f dans un TPoly<float>
  cout<<((TPoly<float>)2.0f)*p1; // utilisation explicite du ctor à 1 paramètre TCoef pour 2.0f
  TPoly<int> p11(1,1), ptri(1);
  for(unsigned deg=1; deg<30; deg++)
    cout<<"Triangle de Pascal degre "<<deg<<""<<(ptri*=p11);
  }
  {
  TPoly<char> pnull, p0(5), p1(2, -4);
  cout<<pnull<<p0<<p1;
  }
}
template<typename TCoef, size_t SZ> void EvalPoly(const TPoly<TCoef> (&TabPoly)[SZ], TCoef v)
{
...
template<size_t SZ> void EvalPoly(const TPoly<char> (&TabPoly)[SZ], char v)
{
...
{
  TPoly<TCoef> poly[]={TPoly<TCoef>(5), TPoly<TCoef>(1, 2), TPoly<TCoef>(1, -4, 1), TPoly<TCoef>(0, 12, 3, 2), TPoly<TCoef>(0, 0, -5, 0, 8)};
  EvalPoly(poly,v);
  cout<<"\n";
}
